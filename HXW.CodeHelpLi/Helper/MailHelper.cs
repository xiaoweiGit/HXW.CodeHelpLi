...
namespace DZNY.Com
{
    public class MailHelper
    {
        ...
        private string mSenderAddress;   //发件箱地址
        ...
        public MailHelper(string toMail,string subject, string emailBody,bool sslEnable, bool pwdCheckEnable)
        {
            try
            {
                ...
                {
                   var toMails = toMail.TrimEnd(';').Split(';');
                   foreach (var to in toMails)
                   {
                       mMailMessage.To.Add(to);
                   }
                }
                this.mSenderServerHost = System.Configuration.ConfigurationManager.AppSettings["mailServerHost"];
                this.mSenderUsername = System.Configuration.ConfigurationManager.AppSettings["mailUsername"];
                this.mSenderPassword = System.Configuration.ConfigurationManager.AppSettings["mailPassword"];
                this.mSenderPort = Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["mailPort"]);
                this.mSenderAddress=System.Configuration.ConfigurationManager.AppSettings["fromMailAddress"];

                mMailMessage.From = new MailAddress(mSenderAddress, mSenderUsername);
        ...
        public MailHelper(string server, string toMail, string fromMail, string subject, string emailBody, string username, string password, string port, bool sslEnable, bool pwdCheckEnable)
        {
            try
            {
                ...
                mMailMessage.From = new MailAddress(fromMail, username);
                mMailMessage.Subject = subject;
                mMailMessage.Body = emailBody;
                mMailMessage.IsBodyHtml = true;
                mMailMessage.BodyEncoding = System.Text.Encoding.UTF8;
        ...
        public bool Send()
        {
            ...
            try
            {
                if (mMailMessage != null)
                {
                    ...
                    {
                        System.Net.NetworkCredential nc = new System.Net.NetworkCredential(this.mSenderAddress, this.mSenderPassword);
                    ...
                    {
                        mSmtpClient.Credentials = new System.Net.NetworkCredential(this.mSenderAddress, this.mSenderPassword);
                    }
