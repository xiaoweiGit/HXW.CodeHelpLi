...
namespace  HXW.CodeHelpLi.Net.Tcp
{
    /// <summary>
    /// TCP服务器基类
    /// </summary>
        ...
        where TSession: TcpSession,new()
    {
        ...
        public void AtomSend(TSession session, byte[] data, int startIndex, int length)
        {
            try
            {
                ...
                Net.NetDebuger.PrintDebugMessage(session, "-MB Session send data count:" + length);
                if (session.IsDebug)
                {
                    ...
                    {
                        result += ",0x" + Convert.ToString(data[i], 16);
                        bytestr += "," + Convert.ToString(data[i]);
                    }

                    NetDebuger.PrintDebugMessage(session, "-Raw send-data(16):" + result);
                    NetDebuger.PrintDebugMessage(session, "-Orgin send-Data(byte):" + bytestr);
                }
            }
            catch (ObjectDisposedException)
            {
